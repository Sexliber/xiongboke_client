  ...
  <div class="container" :class="{loading:isLoading}">
    <vue-scroll ref="vuescroll">
      <div class="content">
        ...
        <!-- 漫画封面 -->
        <div class="comicCover clearboth" v-if="!reqErr">
          <div class="cover">
          ...
          <div class="info">
            ...
            <div class="bt-group">
              <router-link
                ...
                :title="!localStorage.chaperName?comicCatalog[0].num:localStorage.chaperName"
              >
                <span>{{!localStorage.picUrl?'开始阅读':'继续阅读'}}</span>&nbsp;
                <span>{{localStorage.chaperName | undefinedTo(comicCatalog[0].num)}}</span>
              </router-link>
            </div>
          </div>
        </div>
...
export default {
  ...
  created() {
    ...
    if (sub != -1) {
      this.localStorage = json.history[sub];
    }
  }
};
</script>
